...
class IntegratedVoiceInpainting:
    ...
    def _prepare_context_segments(
        ...
        if start_idx > 0:
            ...
            if tokenized_audio.token_to_text_map:
                ...
                if start_char_pos is not None and end_char_pos is not None:
                    ...
                    pre_context_text = tokenized_audio.text[
                        start_char_pos : end_char_pos + 1
                    ]
        ...
        if end_idx < tokenized_audio.rvq_tokens.shape[1]:
            ...
            post_context_audio = self.tokenizer.extract_context_audio(
                tokenized_audio, (end_idx, post_end_idx), context_seconds
            )
            ...
            if tokenized_audio.token_to_text_map:
                ...
                if start_char_pos is not None and end_char_pos is not None:
                    # Extract text subsequence
                    post_context_text = tokenized_audio.text[
                        start_char_pos : end_char_pos + 1
                    ]
    ...
    def _estimate_audio_length(
        self, edit_text: str, tokenized_audio: TokenizedAudio
    ) -> int:
        ...
        try:
            ...
            if (
                tokenized_audio.word_timestamps
                and len(tokenized_audio.word_timestamps) > 3
            ):
                total_chars = 0
                total_duration_ms = 0
                ...
                if total_chars > 10 and total_duration_ms > 0:
                    ...
                    if 40 <= ms_per_char <= 200:
                        logger.info(
                            f"Detected speaking rate: {ms_per_char:.1f}ms per character"
                        )
    ...
    def inpaint(
        ...
        """
        logger.info(
            f"Performing integrated voice inpainting for: '{edit_op.edited_text}'"
        )
        ...
        max_audio_length_ms = self._estimate_audio_length(
            edit_op.edited_text, tokenized_audio
        )
        ...
        with torch.inference_mode():
            ...
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_file:
                ...
                try:
                    ...
                    logger.warning(
                        f"Streaming encode failed: {e}, falling back to regular encode"
                    )
            ...
            inpainted_tokens = torch.cat(
                [
                    tokenized_audio.rvq_tokens.to(new_tokens.device)[:, :start_idx],
                    new_tokens,
                    tokenized_audio.rvq_tokens.to(new_tokens.device)[:, end_idx:],
                ],
                dim=1,
            )
    ...
    def batch_inpaint(
        ...
        for i, edit_op in enumerate(sorted_edits):
            logger.info(
                f"Processing edit {i + 1}/{len(sorted_edits)}: '{edit_op.original_text}' -> '{edit_op.edited_text}'"
            )
            ...
            new_length = inpainted_tokens.shape[1] - (
                current_tokens.shape[1] - old_length
            )
